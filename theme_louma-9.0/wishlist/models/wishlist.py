...
class website(osv.osv):
    ...
    def get_product_wishlist(self, cr, uid, context=None):
        ...
        new_product_ids = []
        reverse_product_ids =[]
        cr.execute("select product_id from wishlist_partner_product_rel where user_id = %s"%(SUPERUSER_ID))
        product_ids = [ i[0] for i in cr.fetchall()]
        for p_id in product_ids:
            product_id = self.pool.get('product.product').browse(cr,SUPERUSER_ID,p_id)
            if product_id:
    ...
    def check_product_is_in_cart(self, cr, uid, product_id, context=None):
        ...
        if order:
            sale_order_lines = sale_order_obj.search(cr, SUPERUSER_ID, [('order_id','=',order.id)])
            for sale_order_line in sale_order_lines:
                p_id = sale_order_obj.browse(cr, SUPERUSER_ID, sale_order_line).product_id.id
                if product_id == p_id:
                    return True
        return False
        ...
        if pricelist:
            ...
            if isinstance(pricelist, basestring):
                pricelist_ids = plobj.name_search(
                    cr, SUPERUSER_ID, pricelist, operator='=', context=context, limit=1)
            ...
            if isinstance(pricelist, (int, long)):
                ...
                pl = plobj.browse(cr, SUPERUSER_ID, pricelist, context=context)
                price = plobj._price_get_multi(cr,SUPERUSER_ID, pl, qtys, context=context)
                for id in ids:
